...
import { ChefHat, Clock, Flame, Lock, LockOpen, RefreshCw } from "lucide-react"
            ...
            <CardFooter className="p-4 pt-0 mt-auto">
                <div className="w-full space-y-2">
                    ...
                    <p className="text-center text-xs text-muted-foreground">
                        {confidence ? `${confidence} confidence` : "Generated meal"}
                    </p>
                    {onLockChange && onReplace ? (
                        <div className="grid grid-cols-2 gap-2">
                            <Button variant="outline" size="sm" disabled={busy} onClick={() => onLockChange(!locked)}>
                                {locked ? <Lock className="mr-1 h-3.5 w-3.5" /> : <LockOpen className="mr-1 h-3.5 w-3.5" />}
                                {locked ? "Locked" : "Lock"}
                            </Button>
                            <Button variant="ghost" size="sm" disabled={busy || locked} onClick={onReplace}>
